...
function buildQuery(input) {
  const facets = (input.get("facets") ?? "").split(",").filter((f) => FACETS.includes(f));
...
async function carFacets(query) {
  ...
  try {
    ...
    for (const name of FACETS) {
      ...
      facets[name] = values.filter((v) => v && v.value !== void 0).slice(0, MAX_VALUES).map((v) => ({ value: String(v.value), count: Number(v.count) || 0 }));
    }
...
function loadCatalog() {
  ...
  cache2 = [...merged.values()].sort((a, b) => a.name.localeCompare(b.name, "es"));
  return cache2;
}
